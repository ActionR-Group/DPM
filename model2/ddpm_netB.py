...
# from .Strans import STTransformer
    ...
    def __init__(self, in_channels, out_channels, rel_reduction=8, mid_reduction=1):
        ...
        for m in self.modules():
            if isinstance(m, nn.Conv2d):
                ...
                bn_init(m, 1)
        self.eca = eca_layer(self.rel_channels, 9)

    def forward(self, x, A=None, alpha=1):
        x1, x2, x3 = self.conv1(x).mean(-2), self.conv2(x).mean(-2), self.conv3(x)
        x1 = x1.unsqueeze(-1) - x2.unsqueeze(-2)  # N C' V V
        # x1 = self.conv4(x1) * alpha + (A.unsqueeze(0).unsqueeze(0) if A is not None else 0)  # N,C,V,V
        x1 = self.eca(x1)
        x1 = self.conv4(x1)
        x1 = self.tanh(x1) * alpha + (A.unsqueeze(0).unsqueeze(0) if A is not None else 0)
...
class unit_gcn(nn.Module):  # spatial modeling
    def __init__(self, in_channels, out_channels, A, coff_embedding=4, adaptive=True, residual=True):
        ...
        if self.adaptive:
            ...
            # self.mask = nn.Parameter(torch.ones(self.PA.shape))
        else:
...
class Modelys(nn.Module):
    def __init__(self, num_class=60, num_point=25, num_person=1, graph=None, graph_args=dict(), in_channels=3,
        ...
        self.l10 = TCN_GCN_unit(base_channel * 4, base_channel * 4, A,
                                adaptive=adaptive)  # 一个TCN_GCN_UNIT是一个basic block
